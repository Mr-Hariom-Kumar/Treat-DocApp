...
const Sidebar = () => {
  ...
  return (
    <div className='min-h-screen bg-white border-right shadow-2xl'>
      {
        atoken &&
        <ul className='mt-5'>
          <NavLink to={'admin-dashboard'} className={({isActive})=>`flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive?'bg-[#F2F3FF] border-r-4 border-primary': ''}`}>
           ...
           <NavLink to={'/add-doctor'} className={({isActive})=>`flex gap-3 items-center py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive?'border-r-4 border-primary bg-[#F2F3FF]':''}`}>
            <img src={assets.add_icon} alt="" />
            <p>Add Doctors</p>
          </NavLink>

          <NavLink to={'/doctor-list'} className={({isActive})=>`flex gap-3 py-3.5 px-3 md:px-9 md:min-w-72 items-center cursor-pointer ${isActive?'border-r-4 border-primary bg-[#F2F3FF]':''}`}>
