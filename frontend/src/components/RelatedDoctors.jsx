    ...
    <div>
      ...
      <div className='w-full grid [grid-template-columns:repeat(auto-fill,minmax(200px,1fr))] gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
            {relDoc.slice(0,10).map((item,index)=>(
                <div key={index} onClick={()=>{navigate(`/appointment/${item._id}`);scrollTo(0,0)}}  className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] translate-all duration-500'>
